...
CreateThread(function()
    while true do
        ...
        for k, v in pairs(Config.sortie) do
            ...
            if dist <= 2 then
                interval = 0
                ESX.ShowHelpNotification("Appuyer sur [~b~E~s~] pour accéder au garage")
                if IsControlJustPressed(1, 51) then
                    ESX.TriggerServerCallback("skn:vehiclelist", function(ownedCars)
        ...
        for k, v in pairs(Config.rentrer) do
            ...
            if dist <= 2 then
                interval = 0
                ESX.ShowHelpNotification("Appuyer sur [~b~E~s~] pour ranger votre véhicule")
                if IsControlJustPressed(1, 51) then
                    ReturnVeh()
                end
            end
...
function SpawnVehicle(vehicle, plate)
    ...
    TriggerServerEvent("skn:breakveh", plate, false)
end
...
function ReturnVeh()
    ...
    if IsPedInAnyVehicle(playerPed, false) then 
        ...
        ESX.TriggerServerCallback("skn:returnveh", function(valid)
            if valid then 
...
function BreakReturnVehicle(vehicle, Propsvehicle)
	...
	TriggerServerEvent('skn:breakveh', Propsvehicle.plate, true)
	ESX.ShowNotification("Tu viens de ranger ton ~o~véhicule ~s~!")
end
