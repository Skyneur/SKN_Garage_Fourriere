...
ESX.RegisterServerCallback("skn:vehiclelist", function(source, cb)
    local ownedCars =  {}
...
RegisterServerEvent("skn:breakveh")
AddEventHandler("skn:breakveh", function(plate, state)
    local _src = source
...
ESX.RegisterServerCallback('skn:returnveh', function (source, cb, Propsvehicle)
	local ownedCars = {}
		...
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle WHERE owner = @owner AND plate = @plate', {
					...
					if rowsChanged == 0 then
						print(('skn_garage : tente de ranger un véhicule non à lui '):format(xPlayer.identifier))
					end
					cb(true)
				end)
			else
...
ESX.RegisterServerCallback("skn:vehiclelistPound", function(source, cb)
	local ownedCars = {}
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND `stored` = @stored", {
...
ESX.RegisterServerCallback("skn:BuyPound", function(source, cb)
	local _src = source
	local xPlayer = ESX.GetPlayerFromId(_src)
	...
	if xPlayer.getMoney() >= Config.poundprice then
		...
		TriggerClientEvent('esx:showNotification', _src, "~r~Vous n'avez pas assez d'argent ~s~!")
		cb(false)
	end
end)